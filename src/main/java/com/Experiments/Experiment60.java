...
/**
 * 这个方法用vmd的解码帧方法每隔1000帧读取1帧。
 *
 */
public class Experiment60 {
    public static final Log LOG = LogFactory.getLog(Experiment60.class);
    ...
    public static void main(String[] args) throws IOException {
      ...
      //创建一个Container来打开视频，如果path是hdfs的路径，则使用HDFSProtocolHandler
        ...
        fw1 = new FileWriter("/home/b8311/Experiment/ExperimentResult/"+"Experiment60.txt",true);
        for(int j=0;j<10;j++){
            String Path = Paths[j];
            Long t=0L;            
            FileWriter fw;
            //fw = new FileWriter("F:\\result\\"+"Experiment11-"+new Path(Path).getName() + ".txt",true);
            fw = new FileWriter("/home/b8311/Experiment/ExperimentResult/"+"Experiment60-"+new Path(Path).getName() + ".txt",true);
            ...
            for(int k=0;k<10;k++){
                ...
                Long framebefore = 0L;
                sw0.start();
                Results results=ClientAPI.GET_FRAME_Index_By_FrameNo(Path,String.valueOf(targetFrame));
                before= ((Long[])(results.getResults()))[0];
                framebefore =((Long[])(results.getResults()))[1];
        ...
        fw1.write(""+totaltime+" "+(double)totaltime/100+"\r\n");
        fw1.close();        
    }
}
